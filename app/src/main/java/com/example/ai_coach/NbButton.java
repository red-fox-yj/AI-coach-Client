...
public class NbButton extends androidx.appcompat.widget.AppCompatButton {
    ...
    private void init(Context context) {
        ...
        int colorDrawable= ContextCompat.getColor(context, R.color.firebrick);
        backDrawable.setColor(colorDrawable);
        backDrawable.setCornerRadius(300);
        ...
        paint.setTextSize(2);
        setText("➜");
        this.setTextColor(Color.parseColor("#faf0e6"));
    }
    ...
    protected void onDraw(final Canvas canvas) {
        super.onDraw(canvas);
        if (isMorphing){
            final RectF rectF=new RectF(getWidth()*4/12,getHeight()/7,getWidth()*8/12,getHeight()-getHeight()/7);
